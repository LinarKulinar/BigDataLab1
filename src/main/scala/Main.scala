...
object Main {
  def main(args: Array[String]) {
    ...
    //val textFile = sc.textFile("file:///C://Users//Kulinar//Desktop//21.txt")
    //val textFile = sc.textFile("file:///C://Users//Kulinar//Desktop//21.txt")
    //C:\Users\Kulinar\Desktop\Новая папка
...
//    val sc = new SparkContext(cfg)
//
//    val tripData = sc.textFile("file:///C://Users//Kulinar//Desktop//big_data//data//trips.csv")
//    // запомним заголовок, чтобы затем его исключить
//    val tripsHeader = tripData.first
//    val trips = tripData.filter(row=>row!=tripsHeader)
//      .map(row=>row.split(",",-1))
//    val stationData = sc.textFile("file:///C://Users//Kulinar//Desktop//big_data//data//stations.csv")
...
//    sc.stop()
    ...
    val sc = new SparkContext(cfg)
//
    val tripData = sc.textFile("file:///C://Users//Kulinar//Desktop//big_data//data//trip.csv")
    val tripsHeader = tripData.first
    val trips = tripData.filter(row=>row!=tripsHeader)
      .map(row=>row.split(",",-1))
    val stationData = sc.textFile("file:///C://Users//Kulinar//Desktop//big_data//data//station.csv")
    val stationsHeader = stationData.first
    val stations = stationData.filter(row=>row!=stationsHeader)
      .map(row=>row.split(",",-1))
